...
class FullyConnected:
    ...
    def __init__(self, N_in, N_h1, N_h2, N_out, device='cpu'):
        ...
        w1 = torch.Tensor().new_empty((N_h1, N_in)).uniform_(-1/(N_h1**0.5), 1/(N_h1**0.5))
        w2 = torch.Tensor().new_empty(N_h2, N_h1).uniform_(-1/(N_h2**0.5), 1/(N_h2**0.5))
        w3 = torch.Tensor().new_empty(N_out, N_h2).uniform_(-1/(N_out**0.5), 1/(N_out**0.5))
        self.weights = {'w1': w1, 'w2': w2, 'w3': w3}

        b1 = torch.Tensor().new_full((N_h1,), 0.01)
        b2 = torch.Tensor().new_full((N_h2,), 0.01)
        b3 = torch.Tensor().new_full((N_out,), 0.01)
        self.biases = {'b1': b1, 'b2': b2, 'b3': b3}
        
        z1 = torch.zeros(N_h1)
        z2 = torch.zeros(N_h2)
        z3 = torch.zeros(N_out)
    ...
    def predict(self, inputs):
        ...
        """
        outputs = self.forward(inputs)# forward pass
        score, idx = torch.max(outputs, dim = 1)# find max score and its index
        return score, idx
    ...
    def accuracy(self, outputs, labels):
        ...
        accuracy = float(torch.sum(torch.eq(labels, predicted_labels)))/float(labels.size()[0])
    ...
    def forward(self, inputs):
        ...
        self.cache['z1'] = activation.sigmoid(self.weighted_sum(inputs, w1, b1))
        a1 = self.cache['z1']
        self.cache['z2'] = activation.sigmoid(self.weighted_sum(a1, w2, b2))
        a2 = self.cache['z2']
        self.cache['z3'] = activation.softmax(self.weighted_sum(a2, w3, b3))
    ...
    def weighted_sum(self, X, w, b):
        ...
        result = torch.add(torch.sum(torch.mul(X.unsqueeze(1), w.unsqueeze(0)), 2), b)
        return result
    ...
    def backward(self, inputs, labels, outputs):
        ...
        #deb_h2a is derivation of error(loss) w.r.t hidden layer 2 activation
        deb_h2a = torch.mul(dout.unsqueeze(2), w3.unsqueeze(0).expand(batch_size,-1,-1))
        #detotal_h2 is derivation of total error(loss) w.r.t hidden layer 2 activation
        detotal_h2 = torch.sum(deb_h2a, dim =1)
        d2 = torch.mul(detotal_h2, activation.delta_sigmoid(self.weighted_sum(a1, w2, b2)))

        #detotal_h1 is derivation of total error(loss) w.r.t hidden layer 1 activation
        detotal_h1 = torch.mul(d2.unsqueeze(2), w2.unsqueeze(0).expand(batch_size, -1, -1))
        detotal_h1 = torch.sum(detotal_h1, 1)
        d1 = torch.mul(detotal_h1, activation.delta_sigmoid(self.weighted_sum(inputs, w1, b1)))
    ...
    def calculate_grad(self, inputs, d1, d2, dout):
        ...
        dw3 = torch.sum(torch.mul(dout.unsqueeze(2), a2.unsqueeze(1)), 0)
        dw2 = torch.sum(torch.mul(d2.unsqueeze(2), a1.unsqueeze(1)), 0)
        dw1 = torch.sum(torch.mul(d1.unsqueeze(2), inputs.unsqueeze(1)), 0)
